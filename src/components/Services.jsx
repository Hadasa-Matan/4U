...
function Services() {
  ...
  // הזרקת הפונט כדי שלא ייעלם במעבר בין עמודים
  const fontFaceStyle = `
  ...
  const otherServices = [
    ...
    },
    {
      ...
      description: "חיבור בין מערכות וטפסים חכמים שמבצעים פעולות במקומך. פחות עבודה ידנית - יותר זמן לצמוח.",
  ...
  return (
    <div className="min-h-screen bg-white" dir="rtl" style={fontRegular}>
      ...
      {/* Hero Section - יישור מרווחים ל-About */}
      <section className="pt-24 pb-8 md:pt-28 md:pb-10 px-6 bg-slate-50/50 border-b border-slate-100 text-center">
      ...
      <section className="py-12 px-6">
        <div className="max-w-7xl mx-auto space-y-12">
          ...
          <motion.div
            ...
            className="rounded-[2.5rem] p-8 md:p-12 border bg-white shadow-xl shadow-blue-900/5"
            style={{ borderColor: `${BRAND_BLUE}20` }}
          >
            <div style={{ color: BRAND_BLUE }} className="mb-6"><BsChatDots className="w-10 h-10" /></div>
            <h3 className="text-2xl md:text-3xl mb-4 text-slate-900" style={fontBold}>שקבוט - המוצר המבוקש לשקט תפעולי</h3>
            <p className="text-slate-600 text-base md:text-lg mb-8 max-w-4xl leading-relaxed">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {chatbotExamples.map((ex, idx) => (
                ...
                >
                  {ex.label} <BsArrowLeft className="w-4 h-4" />
                </button>
              ))}
            </div>
          </motion.div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {otherServices.map((service, idx) => (
              ...
              >
                <div className="mb-6 text-blue-800 bg-slate-50 w-fit p-3 rounded-xl">{service.icon}</div>
                <h3 className="text-xl mb-3 text-slate-900" style={fontBold}>{service.title}</h3>
                <p className="text-sm md:text-base text-slate-600 mb-8">{service.description}</p>
                <button
                  onClick={() => window.open(service.link, "_blank")}
                  className="mt-auto py-3 rounded-xl font-bold border flex items-center justify-center gap-2 hover:bg-slate-50 transition-all"
                  style={{ borderColor: BRAND_BLUE, color: BRAND_BLUE, ...fontBold }}
                >
                  {service.btnText} <BsArrowLeft className="w-4 h-4" />
                </button>
              </motion.div>
            ))}
          </div>
          ...
          {/* CTA - הנעה לפעולה */}
          <div className="text-center bg-slate-50 border p-10 rounded-[3rem] mt-16" style={{ borderColor: `${BRAND_BLUE}10` }}>
            <h4 className="text-2xl md:text-3xl mb-6 text-slate-900" style={fontBold}>
            ...
            <a
              ...
              className="inline-flex items-center gap-3 px-10 py-5 rounded-full font-bold text-xl shadow-xl hover:scale-105 transition-all"
              style={{ backgroundColor: BRAND_GREEN, color: "#06233a", ...fontBold }}
            >
              <BsCalendarCheck className="w-6 h-6" /> שיחת הצעה ללא עלות
            </a>
          </div>
        </div>
      </section>
